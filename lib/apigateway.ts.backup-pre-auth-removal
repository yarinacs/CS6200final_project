...
export class SwnApiGateway extends Construct {
  ...
  constructor(scope: Construct, id: string, props: SwnApiGatewayProps) {
    ...
    this.createBasketApi(props.basketMicroservice, props.userPool);
    this.createOrderApi(props.orderingMicroservices, props.userPool);
  ...
  private createBasketApi(basketMicroservice: IFunction, userPool: IUserPool) {
    ...
    const authorizer = new CognitoUserPoolsAuthorizer(this, 'BasketApiAuthorizer', {
      cognitoUserPools: [userPool]
    });

    const basket = apigw.root.addResource('basket');
    basket.addMethod('GET', undefined, {
      authorizer,
      authorizationType: AuthorizationType.COGNITO,
    });
    basket.addMethod('POST', undefined, {
      authorizer,
      authorizationType: AuthorizationType.COGNITO,
    });

    const singleBasket = basket.addResource('{userName}');
    singleBasket.addMethod('GET', undefined, {
      authorizer,
      authorizationType: AuthorizationType.COGNITO,
    });
    singleBasket.addMethod('DELETE', undefined, {
      authorizer,
      authorizationType: AuthorizationType.COGNITO,
    });

    const basketCheckout = basket.addResource('checkout');
    basketCheckout.addMethod('POST', undefined, {
      authorizer,
      authorizationType: AuthorizationType.COGNITO,
    });
  }

  private createOrderApi(orderingMicroservices: IFunction, userPool: IUserPool) {
    ...
    const authorizer = new CognitoUserPoolsAuthorizer(this, 'OrderApiAuthorizer', {
      cognitoUserPools: [userPool]
    });

    const order = apigw.root.addResource('order');
    order.addMethod('GET', undefined, {
      authorizer,
      authorizationType: AuthorizationType.COGNITO,
    });

    const singleOrder = order.addResource('{userName}');
    singleOrder.addMethod('GET', undefined, {
      authorizer,
      authorizationType: AuthorizationType.COGNITO,
    });
